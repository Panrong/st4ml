...
object testRangeQuery extends App {
  override def main(args: Array[String]): Unit = {
     ...
     */
    val gridNumOverLat = rg.gridNumOverLat
    //println(rg.gridNum)
    //println(rg.gridNumOverLon)
    //println(rg.gridNumOverLat)
    val gridVertexMap = rg.grid2Vertex.map { case (k, v) => (k.x * gridNumOverLat + k.y, v.map(p => Point(p.lon, p.lat, ID = p.id.toLong))) } // k: grid index, v: Array[Point]
  ...
  for(i <- r){
    ...
    */
    ...
    val gridBoundary = rg.grids.map { case (k, v) => (k.x * gridNumOverLat + k.y, Rectangle(Point(v.bottomLeftLon, v.bottomLeftLat), Point(v.upperRightLon, v.upperRightLat), ID = (k.x * gridNumOverLat + k.y).toLong)) }
