...
object odQuery {
  ...
  def strictQuery(queryRDD: RDD[String], odRDD: RDD[(String, Array[String])]): Array[Array[String]] = { // multiple queries
    queryRDD.map(x => (x, 1)).join(odRDD).map(x => x._2._2).collect
  }
