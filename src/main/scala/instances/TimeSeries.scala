...
class TimeSeries[V, D](
  ...
  // assumes disjoint temporal durations
  def allocate(geomArr: Array[Geometry], timestampArr: Array[Long]): TimeSeries[Array[Geometry], D] = {
    ...
    val newValues = entries.zipWithIndex.map(entryWithIdx =>
      entryWithIdx._1.value match {
        case old: Array[Geometry] => old ++ entryIndexToGeom(entryWithIdx._2)
//        case None => entryIndexToGeom(entryWithIdx._2)
        case _ => throw new Exception("cannot allocate TimeSeries with Entry.value's type other than Array[Geometry]")
      }
    )
    val newSpatials = newValues.map(geomArr => Extent(geomArr.map(_.getEnvelopeInternal)).toPolygon)
...
object TimeSeries {
  def empty(durArr: Array[Duration]): TimeSeries[None.type, None.type] = {
    val durArrSorted = durArr.sortBy(_.start)
    TimeSeries(durArrSorted.map(x => Entry(Polygon.empty, x)))
  }
