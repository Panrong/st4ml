...
class SMExtractor extends Extractor with Serializable {
   ...
   */
  def extractRoadSpeed(rdd: RDD[RoadMap[Array[(Long, String, Double)]]]):
  Map[String, Double] = {
   ...
   * @return Map of (roadID, avgSpeed)
   */
  def extractRoadFlow(rdd: RDD[RoadMap[Array[(Long, String, Double)]]]): Map[String, Int] = {
   ...
   */
  def extractSlidingFlow(rdd: RDD[RoadMap[Array[(Long, String, Double)]]],
  ...
  def extractSlidingSpeed(rdd: RDD[RoadMap[Array[(Long, String, Double)]]],
                          startTime: Long, endTime: Long, windowSize: Int):
