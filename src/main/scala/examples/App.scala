...
object App {
  def main(args: Array[String]): Unit = {
    ...
    val filteredPointRDD = rdd2.filter(x => x.coordinates(0) > 115 && x.coordinates(1) > 25 && x.coordinates(0) < 125 && x.coordinates(1) < 35)
    println(" ... Top 5 most frequent:")
    extractor.extractMostFrequentPoints("tripID", 5)(rdd2).foreach(x => println(s" ....  $x"))
