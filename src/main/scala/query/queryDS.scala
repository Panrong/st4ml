...
import org.apache.spark.sql.functions.{size, col, collect_list}
import preprocessing.{SparkSessionWrapper, readTrajFile, readQueryFile}
...
object queryDS extends App {
  ...
  override def main(args: Array[String]): Unit = {
    ...
    /** generate query RDD */
    val queries = readQueryFile(queryFile)
    import spark.implicits._
    val queryDS = queries.map(x => (x, x.ID)).toSeq.toDS()
