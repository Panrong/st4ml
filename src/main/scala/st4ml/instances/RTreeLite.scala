...
abstract class RTreeLiteEntry {
  def minDist(x: Geometry): Double

  def intersect(x: Geometry): Boolean
}

case class RTreeLiteLeafEntry[T <: Geometry : ClassTag](shape: T, mData: String, size: Int) extends RTreeLiteEntry {
  override def minDist(x: Geometry): Double = shape.distance(x)

  override def intersect(x: Geometry): Boolean = x.intersects(shape)
}

case class RTreeLiteInternalEntry(mbr: Polygon, node: RTreeLiteNode) extends RTreeLiteEntry {
  override def minDist(x: Geometry): Double = mbr.distance(x)

  override def intersect(x: Geometry): Boolean = x.intersects(mbr)
}

case class RTreeLiteNode(mMbr: Polygon, mChild: Array[RTreeLiteEntry], isLeaf: Boolean) {
  val size: Long = {
    if (isLeaf) mChild.map(x => x.asInstanceOf[RTreeLiteLeafEntry[Geometry]].size).sum
    else mChild.map(x => x.asInstanceOf[RTreeLiteInternalEntry].node.size).sum
  }
}

object RTreeLiteNode {
  def apply(mMbr: Polygon, children: Array[(Polygon, RTreeLiteNode)]): RTreeLiteNode = {
    RTreeLiteNode(mMbr, children.map(x => RTreeLiteInternalEntry(x._1, x._2)), isLeaf = false)
  }

  def apply[T <: Geometry : ClassTag](mMbr: Polygon, children: => Array[(T, String)]): RTreeLiteNode = {
    RTreeLiteNode(mMbr, children.map(x => RTreeLiteLeafEntry(x._1, x._2, 1)), isLeaf = true)
  }

  def apply[T <: Geometry : ClassTag](mMbr: Polygon, children: Array[(T, String, Int)]): RTreeLiteNode = {
    RTreeLiteNode(mMbr, children.map(x => RTreeLiteLeafEntry(x._1, x._2, x._3)), isLeaf = true)
  }
}

case class RTreeLite[T <: Geometry : ClassTag](root: RTreeLiteNode) extends Serializable {
  var numEntries: Int = 0
  //  var count: Int = 0

  def setNumEntries(n: Int): RTreeLite[T] = {
    numEntries = n
    this
  }
  ...
  def range[Q <: Geometry : ClassTag](query: Q): Array[(T, String)] = {
    ...
    val st = new mutable.Stack[RTreeLiteNode]()
    if (root.mMbr.intersects(query) && root.mChild.nonEmpty) st.push(root)
    while (st.nonEmpty) {
      val now = st.pop()
      if (!now.isLeaf) {
        now.mChild.foreach {
          case RTreeLiteInternalEntry(mbr, node) =>
            if (query.intersects(mbr)) st.push(node)
        }
      } else {
        now.mChild.foreach {
          case RTreeLiteLeafEntry(shape: T, mData, _) =>
  ...
  def range(query: Polygon, levelLimit: Int, sThreshold: Double): Option[Array[(T, String)]] = {
    val ans = mutable.ArrayBuffer[(T, String)]()
    val q = new mutable.Queue[(RTreeLiteNode, Int)]()
      ...
      while (q.nonEmpty) {
        ...
        if (cur_node.isLeaf) {
          cur_node.mChild.foreach {
            case RTreeLiteLeafEntry(shape: T, m_data, _) =>
  ...
  def circleRangeCnt(origin: Geometry, r: Double): Array[(Geometry, String, Int)] = {
    ...
    while (st.nonEmpty) {
      ...
      if (!now.isLeaf) {
        ...
        now.mChild.foreach {
          case RTreeLiteLeafEntry(shape, mData, size) =>
  ...
  def circleRangeConj(queries: Array[(Point, Double)]): Array[(Geometry, String)] = {
    ...
    while (st.nonEmpty) {
      ...
      if (!now.isLeaf) now.mChild.foreach {
        ...
        now.mChild.foreach {
          case RTreeLiteLeafEntry(shape, mData, _) =>
  ...
  def kNN(query: Point, k: Int, keepSame: Boolean = false): Array[(Geometry, String)] = {
    ...
    breakable {
      while (pq.nonEmpty) {
        ...
        now._1 match {
          case RTreeLiteNode(_, m_child, isLeaf) =>
            m_child.foreach(entry =>
              if (isLeaf) pq.enqueue((entry, entry.minDist(query)))
              else pq.enqueue((entry.asInstanceOf[RTreeLiteInternalEntry].node, entry.minDist(query)))
            )
          case RTreeLiteLeafEntry(p, m_data, size) =>
            cnt += size
            kNN_dis = now._2
            ans += ((p, m_data))
        }
...
object RTreeLite {
  def apply[T <: Geometry : ClassTag](entries: Array[(T, String, Int)], maxEntriesPerNode: Int, dimension: Int = 2): RTreeLite[T] = {
    val entriesLen = entries.length.toDouble
    if (entriesLen == 0) {
      val root = new RTreeLiteNode(Extent(0, 0, 0, 0).toPolygon, new Array[RTreeLiteEntry](0), false)
      new RTreeLite(root).setNumEntries(0)
    }
    else {
      ...
      val RTreeLiteNodes = mutable.ArrayBuffer[(Polygon, RTreeLiteNode)]()
      grouped.foreach(list => {
        ...
        RTreeLiteNodes += ((mbr, RTreeLiteNode(mbr, list)))
      })

      var curRTreeLiteNodes = RTreeLiteNodes.toArray
      var curLen = curRTreeLiteNodes.length.toDouble
      ...
      def comp(dim: Int)(left: (Polygon, RTreeLiteNode), right: (Polygon, RTreeLiteNode)): Boolean = {
      ...
      def recursiveGroupRTreeLiteNode(entries: Array[(Polygon, RTreeLiteNode)],
                                      curDim: Int, untilDim: Int): Array[Array[(Polygon, RTreeLiteNode)]] = {
        val len = entries.length.toDouble
        val grouped = entries.sortWith(comp(curDim))
          .grouped(Math.ceil(len / dim(curDim)).toInt).toArray
        if (curDim < untilDim) {
          grouped.flatMap(now => recursiveGroupRTreeLiteNode(now, curDim + 1, untilDim))
        } else grouped
      }

      while (!over(dim)) {
        val grouped = recursiveGroupRTreeLiteNode(curRTreeLiteNodes, 0, dimension - 1)
        val tmpNodes = mutable.ArrayBuffer[(Polygon, RTreeLiteNode)]()
        grouped.foreach(list => {
          ...
          val mbr = Extent(min(0), min(1), max(0), max(1)).toPolygon
          if (dimension == 3) mbr.setUserData(Array(min(2), max(2)))
          tmpNodes += ((mbr, RTreeLiteNode(mbr, list)))
        })
        curRTreeLiteNodes = tmpNodes.toArray
        curLen = curRTreeLiteNodes.length.toDouble
        remaining = curLen / maxEntriesPerNode
        for (i <- 0 until dimension) {
          dim(i) = Math.ceil(Math.pow(remaining, 1.0 / (dimension - i))).toInt
          remaining /= dim(i)
      ...
      val mbr = Extent(min(0), min(1), max(0), max(1)).toPolygon
      if (dimension == 3) mbr.setUserData(Array(min(2), max(2)))
      val root = RTreeLiteNode(mbr, curRTreeLiteNodes)
      new RTreeLite(root).setNumEntries(entriesLen.toInt)
    }
  }
}
