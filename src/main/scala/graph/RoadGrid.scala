...
object RoadGrid {
  ...
  def apply(sourceFilePath: String, gridSize: Double = 0.01): RoadGrid = {
    ...
    val minLon: Double = List(
      ...
      edges.flatMap(_.ls.points).map(_.lon).min,
      edges.map(_.midPoint.lon).min
    ).min
    val minLat: Double = List(
      vertexes.map(_.point.lat).min,
      edges.flatMap(_.ls.points).map(_.lat).min,
      edges.map(_.midPoint.lat).min
    ).min
    val maxLon: Double = List(
      vertexes.map(_.point.lon).max,
      edges.flatMap(_.ls.points).map(_.lon).max,
      edges.map(_.midPoint.lon).max
    ).max
    val maxLat: Double = List(
      vertexes.map(_.point.lat).max,
      edges.flatMap(_.ls.points).map(_.lat).max,
      edges.map(_.midPoint.lat).max
    ).max
