...
class RoadGraph(edges: Array[RoadEdge]) extends Serializable {
  ...
  // sourceId should be different from targetId
  def getShortestPathAndLength(sourceId: String, targetId: String): (List[String], Double) = {

      val src = if (sourceId.contains("-")) sourceId.split("-")(1) else sourceId
      val dst = if (targetId.contains("-")) targetId.split("-")(0) else targetId
